...
-- ========================================================
-- [[ HYBRID TOUCH & MOUSE DRAGGING (LOCK DRAG OPTIMIZED) ]]
-- ========================================================
local function MakeDraggable(dragTrigger, frameToMove)
    local dragging, dragInput, dragStart, startPos
    
    Janitor:Add(dragTrigger.InputBegan:Connect(function(input)
        -- Jika pengaturan kunci drag aktif, batalkan inisialisasi seret [1]
        if Library.Settings and Library.Settings.LockDrag then return end
    ...
    Janitor:Add(UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            -- Mencegah proses seret dinamis secara instan [1]
            if Library.Settings and Library.Settings.LockDrag then 
                dragging = false 
                return 
            end
...
function Library:CreateWindow(titleText, subtitleText, customConfig)
    ...
    Library.Settings = {
        ...
        ExternalShape = "Round",
        LockDrag = false -- Default awal: Dragging diperbolehkan (tidak terkunci) [1]
    }
    ...
    -- NOTIFIKASI SAMBUTAN OTOMATIS (WELCOME & TUTORIAL NOTIFICATIONS)
    task.spawn(function()
        task.wait(0.2)
        -- Notifikasi Sambutan Louis Hub (Aktif selama 5 detik) [1]
        Library:CreateNotification("Welcome to Louis Hub", "Happy Exploiting! Enjoy our customized and premium-tuned interface.", 5)
        
        task.wait(5.5)
        -- Notifikasi Rekomendasi Menyesuaikan Layout UI PC/Mobile (Aktif selama 5 detik) [1]
        Library:CreateNotification("Layout Recommendation", "Go to the Settings tab to optimize your UI layout for Mobile or PC display.", 5)
    end)
