...
local CurrentTheme = Themes["Charcoal"]
...
-- ========================================================
local function MakeDraggable(dragTrigger, frameToMove, onDragEnd)
    local dragging, dragInput, dragStart, startPos
    
    Janitor:Add(dragTrigger.InputBegan:Connect(function(input)
        if Library.Settings.DragLocked then return end
        if frameToMove:GetAttribute("DragLocked") then return end
    ...
    Janitor:Add(UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            if frameToMove:GetAttribute("DragLocked") then 
                dragging = false
                return 
            end
...
function Library:CreateExternalButton(text, buttonType, shape, flag, callback)
    local screenGui = game:GetService("CoreGui"):FindFirstChild("Louis_Hub_UI") or game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):FindFirstChild("Louis_Hub_UI")
    ...
    local function savePosition()
        local pos = ExtBtnFrame.Position
        Library.Flags["ExtPos_" .. flag] = {pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset}
    end
    
    MakeDraggable(ExtBtnFrame, ExtBtnFrame, savePosition)
    ...
    function controller:SetVisible(visibleState)
        ExtBtnFrame.Visible = visibleState
    end
    
    function controller:SetText(val)
        ActBtn.Text = tostring(val)
    end
    
    function controller:SetDragLock(locked)
        ExtBtnFrame:SetAttribute("DragLocked", locked)
    end
    
    function controller:SetSize(size)
        ExtBtnFrame.AutomaticSize = Enum.AutomaticSize.None
        ExtBtnFrame.Size = size
    end
...
-- ========================================================
-- [[ BACKEND: TOAST NOTIFICATION MANAGER ]]
-- ========================================================
...
function Library:CreateWindow(titleText, subtitleText, customConfig)
    ...
    HudFrame.Name = "Louis_Performance_HUD"
    ...
    TitleLabel.Text = "Louis Hub"
    TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
    RegisterTheme(TitleLabel, { TextColor3 = "TextPrimary" })
    RegisterFont(TitleLabel, true)
    RegisterText(TitleLabel, 13)

    local ScriptNameLabel = Instance.new("TextLabel", LogoArea)
    ScriptNameLabel.Size = UDim2.new(1, -90, 0, 12)
    ScriptNameLabel.Position = UDim2.new(0, 52, 0, 26)
    ScriptNameLabel.BackgroundTransparency = 1
    ScriptNameLabel.Text = titleText or "Universal Script"
    ScriptNameLabel.TextXAlignment = Enum.TextXAlignment.Left
    RegisterTheme(ScriptNameLabel, { TextColor3 = "TextSecondary" })
    ...
    RegisterTheme(tabSearchStroke, { Color = "StrokeColor" })
    ...
    -- Dynamic Sidebar Scrolling Proximity Easing Animation
    local function UpdateScrollAnimations()
        if not TabScroll.Visible or TabScroll.AbsoluteWindowSize.Y == 0 then return end
        ...
        for _, child in ipairs(TabScroll:GetChildren()) do
            if child:IsA("TextButton") or (child:IsA("Frame") and child.Name ~= "TabBtn") then
                ...
                local scaleObj = child:FindFirstChildOfClass("UIScale")
                if not scaleObj then
                    scaleObj = Instance.new("UIScale", child)
                end
    ...
    SubtextLabel.Text = subtitleText or "MEMBER"
    ...
    -- Prevents overlapping/clutter on compact Mobile layout modes
    ...
    FloatingToggle.Name = "Louis_Floating_Toggler"
    ...
    local function ToggleGui()
        ...
        MainFrame.ClipsDescendants = true -- Enable clipping safely during animations
        ...
        if Window.Visible then
            ...
            local tween = TweenService:Create(MainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = TargetSize, Position = TargetPosition})
            tween:Play()
            tween.Completed:Connect(function()
                if Window.Visible then
                    MainFrame.ClipsDescendants = false -- Disable clipping when open so dropdown lists render properly outside boundaries
                end
            end)
    ...
    task.spawn(function()
        task.wait(0.2)
        Library:CreateNotification("Welcome to Louis Hub", "UI Framework executed successfully. Press Insert to minimize.", 5)
    end)

    -- ========================================================
    -- [[ RIGHT SIDE GENERAL FEATURE SEARCH PANEL ]]
    -- ========================================================
    ...
    local SearchStroke = Instance.new("UIStroke", FeatureSearchFrame)
    ...
    -- Search options panel inner close button
    ...
    ResultsScroll.Position = UDim2.new(0, 5, 0, 50)
    ResultsScroll.BackgroundTransparency = 1
    ResultsScroll.ScrollBarThickness = 3
    RegisterTheme(ResultsScroll, { ScrollBarImageColor3 = "Accent" })

    local resultsLayout = Instance.new("UIListLayout", ResultsScroll)
    resultsLayout.Padding = UDim.new(0, 4)
    resultsLayout.SortOrder = Enum.SortOrder.LayoutOrder

    Janitor:Add(resultsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        ResultsScroll.CanvasSize = UDim2.new(0, 0, 0, resultsLayout.AbsoluteContentSize.Y)
    end))

    local function HighlightElement(gui)
        if not gui then return end
        local originalColor = gui.BackgroundColor3
        local highlightTween = TweenService:Create(gui, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    ...
    local function UpdateSearch()
        ...
        for _, item in ipairs(Library.SearchRegistry) do
            if string.find(string.lower(item.Name), query) then
                local itemBtn = Instance.new("TextButton", ResultsScroll)
                itemBtn.Size = UDim2.new(1, -10, 0, 30)
                itemBtn.Text = item.Name .. " (" .. item.Type .. ")"
                RegisterTheme(itemBtn, { BackgroundColor3 = "ElementBg", TextColor3 = "TextSecondary" })
                RegisterFont(itemBtn, false)
                RegisterText(itemBtn, 10)
                Instance.new("UICorner", itemBtn).CornerRadius = UDim.new(0, 4)
                
                local btnStroke = Instance.new("UIStroke", itemBtn)
                btnStroke.Thickness = 1
                RegisterTheme(btnStroke, { Color = "StrokeColor" })
                
                itemBtn.MouseButton1Click:Connect(function()
                    item.Tab.SelectTab()
                    HighlightElement(item.Gui)
                end)
            end
        end
    end
    Janitor:Add(SearchInput:GetPropertyChangedSignal("Text"):Connect(UpdateSearch))

    function Library:RegisterSearchItem(name, itemType, tabObj, elementGui)
    ...
    function Window:CreateTab(tabName, iconInput, isPremium)
        ...
        local function ResizeCanvas()
            ...
            if Library.Settings.Mode == "PC" then
                ...
                -- Stacked vertical columns on mobile layout to completely prevent overlaps
                ColumnLayout.Enabled = true
                
                LeftColumn.Size = UDim2.new(1, 0, 0, leftHeight)
                LeftColumn.AutomaticSize = Enum.AutomaticSize.None
                
                RightColumn.Visible = false -- Hidden on Mobile layout to prevent dual-column crowding
                
                local targetHeight = leftHeight + 50
                TabPage.CanvasSize = UDim2.new(0, 0, 0, targetHeight)
                ColumnContainer.Size = UDim2.new(1, -20, 0, targetHeight)
            end
        end
        ...
        -- ========================================================
        -- [[ SECTION ELEMENT FACTORY HELPERS ]]
        -- ========================================================
        function Tab._CreateToggleHelper(ContentFrame, toggleText, defaultVal, flag, config, callback)
            local Toggle = { State = defaultVal or false }
            Library.Flags[flag] = Toggle.State

            local Elem = Instance.new("Frame", ContentFrame)
        ...
        function Tab._CreateDropdownHelper(ContentFrame, ddText, options, defaultVal, flag, callback)
            ...
            local function UpdateDropdownPosition()
                local scale = Library.Settings.Scale or 1.0
                local relativeX = (Trigger.AbsolutePosition.X - MainFrame.AbsolutePosition.X) / scale
                local relativeY = (Trigger.AbsolutePosition.Y - MainFrame.AbsolutePosition.Y) / scale
                ListFrame.Position = UDim2.new(0, relativeX, 0, relativeY + (Trigger.AbsoluteSize.Y / scale) + 4)
                ListFrame.Size = UDim2.new(0, Trigger.AbsoluteSize.X / scale, 0, math.min(#options * 23 + 4, 100))
            end
        ...
        function Tab._CreateParagraphHelper(ContentFrame, paraTitle, paraDesc)
            ...
            local function ResizeParagraph()
                local constraintSize = Vector2.new(ContentFrame.AbsoluteSize.X - 20, 1000)
            ...
            Janitor:Add(ContentFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(ResizeParagraph))
            ResizeParagraph()
        ...
        -- ========================================================
        -- [[ DYNAMIC TAB SUB-PAGE INSTANTIATOR ]]
        -- ========================================================
        function Tab:CreatePage(pageName)
            local Page = { Sections = {} }
            
            if not Tab.PageNav then
                Tab.PageNav = Instance.new("Frame", TabPage)
                Tab.PageNav.Size = UDim2.new(1, -20, 0, 30)
                Tab.PageNav.Position = UDim2.new(0, 10, 0, 5)
                Tab.PageNav.BackgroundTransparency = 1
                
                local navLayout = Instance.new("UIListLayout", Tab.PageNav)
                navLayout.FillDirection = Enum.FillDirection.Horizontal
                navLayout.Padding = UDim.new(0, 6)
            end
            
            local PageContainer = Instance.new("Frame", TabPage)
            ...
            Janitor:Add(LeftList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(PageResizeCanvas))
            Janitor:Add(RightList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(PageResizeCanvas))
            
            local PageBtn = Instance.new("TextButton", Tab.PageNav)
            PageBtn.Size = UDim2.new(0, 80, 1, 0)
            PageBtn.BackgroundTransparency = 0.5
            PageBtn.Text = pageName
            RegisterTheme(PageBtn, { BackgroundColor3 = "ElementBg", TextColor3 = "TextSecondary" })
            RegisterFont(PageBtn, true)
            RegisterText(PageBtn, 10)
            Instance.new("UICorner", PageBtn).CornerRadius = UDim.new(0, 4)
            local Stroke = Instance.new("UIStroke", PageBtn)
            Stroke.Thickness = 1
            RegisterTheme(Stroke, { Color = "StrokeColor" })
            
            local function SelectPage()
                for _, p in ipairs(Tab.Pages or {}) do
                    p.Container.Visible = false
                    TweenService:Create(p.Button, TweenInfo.new(0.2), { BackgroundTransparency = 0.5 }):Play()
                    TweenService:Create(p.Button, TweenInfo.new(0.2), { TextColor3 = CurrentTheme.TextSecondary }):Play()
                end
                
                if Tab.DefaultContainer then Tab.DefaultContainer.Visible = false end
                
                PageContainer.Visible = true
                TweenService:Create(PageBtn, TweenInfo.new(0.2), { BackgroundTransparency = 0 }):Play()
                TweenService:Create(PageBtn, TweenInfo.new(0.2), { TextColor3 = CurrentTheme.Accent }):Play()
                PageResizeCanvas()
            end
            ...
            function Page:CreateSection(sectionName)
                ...
                local function ToggleSection()
                    ...
                    if Section.Collapsed then
                        ...
                        TweenService:Create(ToggleIcon, TweenInfo.new(duration, ease), { Rotation = -90 }):Play()
                        shrink.Completed:Connect(function()
                            if Section.Collapsed then Content.Visible = false end
                        end)
                    else
                        Content.Visible = true
                        local contentHeight = ContentList.AbsoluteContentSize.Y
            ...
            if ColumnContainer then
                Tab.DefaultContainer = ColumnContainer
            end
    ...
    task.spawn(function()
        ...
        local function LoadConfig(configName)
            ...
            if loadedData and typeof(loadedData) == "table" then
                for flag, value in pairs(loadedData) do
                    if string.match(flag, "^ExtPos_") then
                        Library.Flags[flag] = value
                        local originalFlag = string.gsub(flag, "^ExtPos_", "")
                        local btnFrame = Library.ExternalButtons[originalFlag]
                        if btnFrame then
                            btnFrame.Position = UDim2.new(value[1], value[2], value[3], value[4])
                        end
                    elseif Library.Registry[flag] then
                        pcall(function()
        ...
        -- Strictly creates the two requested categories and tabs (Setting and Configs)
        ...
        -- CONFIG tab (Configs File Manager Workspace)
        local ConfigManagerTab = Window:CreateTab("CONFIG", "folder")
    ...
    function Library:ShowInfoModal(infoTitle, infoText)
        local oldModal = ScreenGui:FindFirstChild("Louis_Info_Modal")
        if oldModal then oldModal:Destroy() end
        
        local oldOverlay = ScreenGui:FindFirstChild("Louis_Modal_Overlay")
        if oldOverlay then oldOverlay:Destroy() end
        
        local Overlay = Instance.new("TextButton", ScreenGui)
        Overlay.Name = "Louis_Modal_Overlay"
        ...
        ModalFrame.Name = "Louis_Info_Modal"
